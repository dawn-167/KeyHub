...
2. **`.` 键 vs `T` 键**：`.options tnom=0` 这类指令必须用 `.` 键放置；用 T 放的就是注释，仿真时被忽略。

3. **SPICE 指令以 `.` 开头**才合法；注释、空行、`*` 开头行都不会执行。

4. **验证指令是否生效**：跑完仿真 → **Ctrl+L** 看日志。如 `.options tnom=0` 生效，日志会显示 `tnom = 0`（默认 27）。

5. **`.asc` 与 `.net` 是单向转换**：原理图 → 自动生成网表；手写的 .net 内容不会回写原理图，重新生成会**覆盖丢失**。

6. **电流源（I/G 源）输出端必须接负载电阻**到地，否则报 `floating` 错误——电流源不决定节点电压，电压才由负载决定。

7. **受控源字母记忆**：E、G 是**电压控制**（VCVS/VCCS），F、H 是**电流控制**（CCCS/CCVS）；增益是乘法，H 源增益乘的是**电流**（V = 增益 × I）。

8. **`temp` vs `tnom`**：`temp` 是仿真环境温度，`tnom` 是元件参数标定温度（默认 27℃）。想改基准用 `.options tnom=...`。

9. **电容 `ic=1V`（初始条件）**：只在瞬态分析生效，且要配合 `.tran ... UIC` 才会真正作为起点，否则先算直流工作点会覆盖它。

10. **Ctrl+L 的日志文件**同时保存在网表同目录的 `.log` 文件中，可直接用文本编辑器查看。
