...
>
> **绘图临时选项**：画导线时按住 `⌘` 可临时切换正交吸附，允许画任意角度导线；用 Draw > Line/Rectangle/Circle/Arc 绘制图形注释时，按住 `⌘` 可临时禁用网格吸附，实现自由定位。
...
> **原理图探针操作**（仿真运行后，直接在原理图上点击探测波形）：
> 1. 单击导线 → 绘制该节点电压波形
> 2. 单击元件体 → 绘制该元件电流波形
> 3. 单击元件引脚 → 绘制该引脚电流波形
> 4. ⌥+单击导线 → 绘制导线电流（Wire Current）
> 5. ⌥+单击元件 → 绘制瞬时功率（Instantaneous Power）
> 6. 拖拽两个节点 → 绘制差分电压（Differential Voltage）
> 7. 双击同一节点/电流 → 单独显示该轨迹（清除其他所有轨迹）

> **波形平移与光标操作**（在波形窗格内点击激活键盘操作）：
> 1. 方向键（←→↑↓）：无光标时平移 ~25%；有光标时光标吸附到下一个时间数据点
> 2. ⇧+方向键：无光标时平移 ~50%；有光标时光标吸附到下一个数据点
> 3. ⌥+方向键 或 ⌘+方向键：有光标时光标跳 10 个数据点
> 4. ⌘⌥+方向键：有光标时光标跳 100 个数据点
> 5. 按住 ⌘ + 鼠标拖拽：平移视图
> 6. 按住 ⌘⌥ + 鼠标左右拖拽：左右平移
> 7. 按住 ⌥⌘ + 鼠标上下拖拽：上下平移
> 8. 有光标时，↑↓ 键可在多组数据（.step/.dc/.temp）间切换光标所属轨迹

---

## 五、符号编辑器绘图（Drawing）

> 用于编辑 .asy 符号文件。选中元件后通过菜单进入符号编辑。
>
...
## 六、其他编辑器（Other Editors）

### 网表编辑器（Netlist Editor）
